...
class Container implements ArrayAccess, ContainerContract
{
    ...
    protected function addDependencyForCallParameter(ReflectionParameter $parameter, array &$parameters, &$dependencies)
    {
        ...
        } elseif ((function ($parameter) {
                if (method_exists($parameter, 'getType')) {
                    $type = $parameter->getType();
                    if ($type instanceof \ReflectionNamedType && !$type->isBuiltin()) {
                        try { return new \ReflectionClass($type->getName()); } catch (\ReflectionException $e) { return null; }
                    }
                    return null;
                }
                return method_exists($parameter, 'getClass') ? \App\Support\Php83\Reflection::classOf($parameter) : null;
            })($parameter)) {
            $dependencies[] = $this->make((function ($parameter) {
                if (method_exists($parameter, 'getType')) {
                    $type = $parameter->getType();
                    if ($type instanceof \ReflectionNamedType && !$type->isBuiltin()) {
                        try { return new \ReflectionClass($type->getName()); } catch (\ReflectionException $e) { return null; }
    ...
    {
        try {
            return $this->make((function ($parameter) {
                if (method_exists($parameter, 'getType')) {
                    $type = $parameter->getType();
                    if ($type instanceof \ReflectionNamedType && !$type->isBuiltin()) {
                        try { return new \ReflectionClass($type->getName()); } catch (\ReflectionException $e) { return null; }
    ...
    protected function getFunctionHint(Closure $callback)
    {
        ...
        if (! \App\Support\Php83\Reflection::classOf($expected)) {
            return;
        }

        return \App\Support\Php83\Reflection::classOf($expected)->name;
    }
